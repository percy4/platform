...
class NSDAccess(object):
    ...
    def __init__(self, nsd_folder, *args, **kwargs):
        ...
        # self.nsd_folder = "C:/Users/ghostw/Code/dcp-dag-export-demo/visual_analysis_demo/data/natural-scenes-dataset"
        print(f"self.nsd_folder: {self.nsd_folder}")
        ...
        self.nsd_folder = "/data/zx/MindBridge/data/natural-scenes-dataset"
    ...
    def download_coco_annotation_file(
        ...
        zip_file_object.extractall(
            path=op.split(op.split(self.coco_annotation_file)[0])[0]
        )
    ...
    def affine_header(self, subject, data_format="func1pt8mm"):
        ...
        """
        full_path = op.join(
            self.ppdata_folder, "{subject}", "{data_format}", "brainmask.nii.gz"
        )
        full_path = full_path.format(subject=subject, data_format=data_format)
    ...
    def read_vol_ppdata(self, subject, filename="brainmask", data_format="func1pt8mm"):
        ...
        """
        full_path = op.join(
            self.ppdata_folder, "{subject}", "{data_format}", "{filename}.nii.gz"
        )
        full_path = full_path.format(
            subject=subject, data_format=data_format, filename=filename
        )
    ...
    def read_betas(
        ...
        """
        data_folder = op.join(
            self.nsddata_betas_folder, subject, data_format, data_type
        )
        si_str = str(session_index).zfill(2)

        if (
            type(mask) == np.ndarray
        ):  # will use the mat file iff exists, otherwise boom!
            ipf = op.join(data_folder, f"betas_session{si_str}.mat")
            assert op.isfile(ipf), (
                "Error: "
                + ipf
                + " not available for masking. You may need to download these separately."
            )
        ...
        if data_format == "fsaverage":
            ...
            for hemi in ["lh", "rh"]:
                hdata = nb.load(
                    op.join(data_folder, f"{hemi}.betas_session{si_str}.mgh")
                ).get_data()
            ...
            out_data = nb.load(
                op.join(data_folder, f"betas_session{si_str}.nii.gz")
            ).get_fdata()
    ...
    def read_mapper_results(
        self, subject, mapper="prf", data_type="angle", data_format="fsaverage"
    ):
        ...
        if data_format == "fsaverage":
            ...
            return self.read_vol_ppdata(
                subject=subject,
                filename=f"{mapper}_{data_type}",
    ...
    def read_atlas_results(self, subject, atlas="HCP_MMP1", data_format="fsaverage"):
        ...
        if data_format not in ("func1pt8mm", "func1mm", "MNI"):
            ...
            if atlas[:3] in (
                "rh.",
                "lh.",
            ):  # check if hemisphere-specific atlas requested
                ipf = op.join(
                    self.nsddata_folder, "freesurfer", subject, "label", f"{atlas}.mgz"
                )
            ...
            ipf = op.join(
                self.ppdata_folder, subject, data_format, "roi", f"{atlas}.nii.gz"
            )
    ...
    def read_behavior(self, subject, session_index, trial_index=[]):
        ...
        behavior = pd.read_csv(
            self.behavior_file.format(subject=subject), delimiter="\t"
        )
    ...
    def read_images(self, image_index, show=False):
        ...
        if show:
            f, ss = plt.subplots(1, len(image_index), figsize=(6 * len(image_index), 6))
    ...
    def read_image_coco_info(
        self, image_index, info_type="captions", show_annot=False, show_img=False
    ):
        ...
        """
        if not hasattr(self, "stim_descriptions"):
            self.stimuli_description_file = "/data/zx/MindBridge/data/natural-scenes-dataset/nsddata/experiments/nsd/nsd_stim_info_merged.csv"
            self.stim_descriptions = pd.read_csv(
                self.stimuli_description_file, index_col=0
            )
        if len(image_index) == 1:
            ...
            # load train_2017
            annot_file = self.coco_annotation_file.format(info_type, "train2017")
            coco_train = COCO(annot_file)

            # also load the val 2017
            annot_file = self.coco_annotation_file.format(info_type, "val2017")
            ...
            for image in image_index:
                ...
                if subj_info["cocoSplit"] == "train2017":
                    coco_annot_IDs = coco_train.getAnnIds([subj_info["cocoId"]])
                    coco_ann = coco_train.loadAnns(coco_annot_IDs)
                    coco_annot.append(coco_ann)

                elif subj_info["cocoSplit"] == "val2017":
                    coco_annot_IDs = coco_val.getAnnIds([subj_info["cocoId"]])
        ...
        if len(image_index) == 1:
            ...
            for cat_id in cat_ids:
                ...
                if coco_id in this_img_list:
                    this_cat = np.asarray(
                        categories[categories["id"] == cat_id]["name"]
                    )[0]
            ...
            # load train_2017
            annot_file = self.coco_annotation_file.format("instances", "train2017")
            coco_train = COCO(annot_file)
            cat_ids_train = coco_train.getCatIds()
            categories_train = json_normalize(coco_train.loadCats(cat_ids_train))

            # also load the val 2017
            annot_file = self.coco_annotation_file.format("instances", "val2017")
            coco_val = COCO(annot_file)
            ...
            for image in tqdm(
                ...
                if subj_info["cocoSplit"] == "train2017":
                    for cat_id in cat_ids_train:
                        ...
                        if coco_id in this_img_list:
                            this_cat = np.asarray(
                                categories_train[categories_train["id"] == cat_id][
                                    "name"
                                ]
                            )[0]
                    ...
                    for cat_id in cat_ids_val:
                        ...
                        if coco_id in this_img_list:
                            this_cat = np.asarray(
                                categories_val[categories_val["id"] == cat_id]["name"]
                            )[0]
                            image_cat.append(this_cat)
