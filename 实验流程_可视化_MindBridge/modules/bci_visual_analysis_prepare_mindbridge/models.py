...
def add_hooks(module, parent_name=""):
    ...
    module.register_forward_hook(
        lambda mod, inp, out: forward_hook(mod, inp, out, module_name)
    )
    module.register_backward_hook(
        lambda mod, inp, out: backward_hook(mod, inp, out, module_name)
    )
...
def backward_hook(module, grad_input, grad_output, name):
    if any(
        tensor is not None and torch.isnan(tensor).any()
...
class Clipper(torch.nn.Module):
    def __init__(
        ...
        model_path = "/data/zx/project_total/MindBridge/clip-vit-large-patch14"
...
class MindBridge(MindSingle):
    ...
    def forward(self, x):
        ...
        # forward cycling
        x_b = self.builder[str(subj_b)](
            x_a
        )  # subj_b recon brain signal using subj_a seman embedding
