...
class Trainer:
    def __init__(
        self, args, accelerator, voxel2clip, clip_extractor, prompts_list, device
    ) -> None:
        # train logs path
        ...
        opt_grouped_parameters = [
            {
                "params": [
                    p
                    for n, p in self.voxel2clip.named_parameters()
    ...
    def prepare_wandb(self, local_rank, args):
        ## Weights and Biases
        if (
            local_rank == 0 and args.wandb_log
    ...
    def train(self, local_rank):
        ...
        ## Main loop
        print(
            f"{self.args.model_name} starting with epoch {epoch} / {self.args.num_epochs}"
        )
        progress_bar = tqdm(
            range(epoch, self.args.num_epochs), disable=(local_rank != 0)
        )
        ...
        for epoch in progress_bar:
            ...
            self.sims_image = 0.0
            self.sims_text = 0.0
            self.val_sims_image = 0.0
            self.val_sims_text = 0.0
            self.fwd_percent_correct = 0.0
            self.bwd_percent_correct = 0.0
            self.val_fwd_percent_correct = 0.0
            self.val_bwd_percent_correct = 0.0
            self.loss_clip_image_sum = 0.0
            self.loss_clip_text_sum = 0.0
            self.loss_mse_image_sum = 0.0
            self.loss_mse_text_sum = 0.0
            self.loss_rec_sum = 0.0
            self.loss_cyc_sum = 0.0
            self.val_loss_clip_image_sum = 0.0
            self.val_loss_clip_text_sum = 0.0
            self.val_loss_mse_image_sum = 0.0
            self.val_loss_mse_text_sum = 0.0
            self.val_loss_rec_sum = 0.0
            self.val_loss_cyc_sum = 0.0
            ...
            # Save model
            if (
                epoch % self.args.ckpt_interval == 0
    ...
    def train_step(self, voxel, image, captions, subj_id):
        ...
        clip_image_pred_norm = nn.functional.normalize(
            clip_image_pred.flatten(1), dim=-1
        )
        ...
        self.sims_image += (
            nn.functional.cosine_similarity(clip_image_norm, clip_image_pred_norm)
            .mean()
            .item()
        )
        self.sims_text += (
            nn.functional.cosine_similarity(clip_text_norm, clip_text_pred_norm)
            .mean()
            .item()
        )
        ...
        self.fwd_percent_correct += utils.topk(
            utils.batchwise_cosine_similarity(clip_image_pred_norm, clip_image_norm),
            labels,
            k=1,
        )
        self.bwd_percent_correct += utils.topk(
            utils.batchwise_cosine_similarity(clip_image_norm, clip_image_pred_norm),
            labels,
            k=1,
        )
    ...
    def eval_step(self, voxel, image, captions, subj_id):
        ...
        with torch.no_grad():
            ...
            # text clip loss
            clip_text_pred = results[1]
            clip_text_pred_norm = nn.functional.normalize(
                clip_text_pred.flatten(1), dim=-1
            )
            clip_text_norm = nn.functional.normalize(clip_text.flatten(1), dim=-1)
            ...
            self.val_fwd_percent_correct += utils.topk(
                utils.batchwise_cosine_similarity(
                    clip_image_pred_norm, clip_image_norm
                ),
                labels,
                k=1,
            )
            self.val_bwd_percent_correct += utils.topk(
                utils.batchwise_cosine_similarity(
                    clip_image_norm, clip_image_pred_norm
                ),
                labels,
                k=1,
            )
    ...
    def save_ckpt(self, tag, epoch):
        ...
        if self.accelerator.is_main_process:
            ...
            try:
                parent_path = os.path.dirname(ckpt_path) + f"/last"
            ...
            #     print("Couldn't save... moving on to prevent crashing.")
                ...
                if os.path.isdir(os.path.dirname(ckpt_path)):
                    tmp_path = "./last.pth"
                    torch.save(
                        {
                            "epoch": epoch,
                    ...
                    print(f"Saved via workaround to {ckpt_path}")
        ...
        try:
            ...
            print("Couldn't save accelerator state... trying fallback.")
            print("Error:", e)
            traceback.print_exc()
            ...
            if os.path.isdir(state_path):
                try:
                    ...
                    self.accelerator.save_state(tmp_state)

                    # 移动所有内容到原目标路径
                    for fname in os.listdir(tmp_state):
                        shutil.move(
                            os.path.join(tmp_state, fname),
                            os.path.join(state_path, fname),
                        )
                    print(f"Accelerator state saved via fallback to {state_path}")
                except Exception as e2:
                    print("Fallback for accelerator state also failed.")
                    print("Error:", e2)
                    traceback.print_exc()
    ...
    def save(self, epoch):
        ...
        # save best model
        current_sim = (
            (self.val_sims_image + self.val_sims_text) / (self.val_i + 1)
            if hasattr(self, "val_i")
            else 0
        )
    ...
    def load(
        ...
        unwrapped_voxel2clip.load_state_dict(
        ...
        self.logs.update(
            {
                ...
                "val/val_fwd_pct_correct": self.val_fwd_percent_correct
                / (self.val_i + 1),
                "val/val_bwd_pct_correct": self.val_bwd_percent_correct
                / (self.val_i + 1),
...
class Trainer_single(Trainer):
    ...
    def prepare_dataloader(self):
        ...
        print("调用 get_dls() 参数如下：")
        print(f"  subject      : {self.args.subj_list[0]}")
        print(f"  data_path    : {self.args.data_path}")
        print(f"  batch_size   : {self.args.batch_size}")
        print(f"  val_batch_size: {self.args.val_batch_size}")
        print(f"  num_workers  : {self.args.num_workers}")
        print(f"  pool_type    : {self.args.pool_type}")
        print(f"  pool_num     : {self.args.pool_num}")
        print(f"  length       : {self.args.length}")
        print(f"  seed         : {self.args.seed}")
        self.train_dl, self.val_dl = data.get_dls(
            subject=self.args.subj_list[0],
            data_path="/data/zx/MindBridge/data/natural-scenes-dataset",
    ...
    def train_epoch(self, epoch):
        ...
        for train_i, data_i in enumerate(self.train_dl):
            # print(f"train_i, data_i： {train_i}, {data_i}")
            ...
            captions = [
                prompts[repeat_index]["caption"] for prompts in current_prompts_list
            ]

            print(
                ">>> Epoch{} | Iter{} | voxel: {}".format(epoch, train_i, voxel.shape),
                flush=True,
            )
    ...
    def eval_epoch(self, epoch):
        ...
        for val_i, data_i in enumerate(self.val_dl):
            ...
            print(
                ">>> Epoch{} | Eval{} | voxel: {}".format(epoch, val_i, voxel.shape),
                flush=True,
            )
...
class Trainer_bridge(Trainer):
    ...
    def prepare_multi_gpu(self):
        self.voxel2clip, self.optimizer, self.lr_scheduler, _ = (
            self.accelerator.prepare(
...
class Trainer_adapt(Trainer):
    ...
    def train_epoch(self, epoch):
        ...
        for train_i, datas_target in enumerate(self.train_dl_target):
            ...
            source_index = train_i % len(
                train_dls_source_iter
            )  # every time choose one source domain
            voxel_source, image_source, coco_source, subj_id_source = next(
                train_dls_source_iter[source_index]
            )
            ...
            print(
                ">>> Epoch{} | Iter{} | source{} | voxel: {}".format(
                    epoch, train_i, source_index, voxel.shape
                ),
                flush=True,
            )
            self.train_step(voxel, image, captions, subj_id)
    ...
    def eval_epoch(self, epoch):
        ...
        for val_i, datas_target in enumerate(self.val_dl_target):
            ...
            source_index = val_i % len(
                val_dls_source_iter
            )  # every time choose one source domain
            print("Using source {}".format(source_index))
            voxel_source, image_source, coco_source, subj_id_source = next(
                val_dls_source_iter[source_index]
            )
